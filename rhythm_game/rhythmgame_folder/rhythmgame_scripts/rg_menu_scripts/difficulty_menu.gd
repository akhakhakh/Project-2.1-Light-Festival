...
func _ready():
	if not easy_button.pressed.is_connected(Callable(self, "_on_easy_button_pressed")):
		easy_button.pressed.connect(Callable(self, "_on_easy_button_pressed"))

	if not medium_button.pressed.is_connected(Callable(self, "_on_medium_button_pressed")):
		medium_button.pressed.connect(Callable(self, "_on_medium_button_pressed"))

	if not hard_button.pressed.is_connected(Callable(self, "_on_hard_button_pressed")):
		hard_button.pressed.connect(Callable(self, "_on_hard_button_pressed"))
...
func _on_easy_button_pressed():
	_play_button_sound()
	get_tree().change_scene_to_file("res://rhythmgame_folder/rhythmgame_scenes/normal_level/game_level.tscn")
